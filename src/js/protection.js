...
(function() {
    ...
    const devtoolsDetector = () => {
        ...
        if (dt > 100) { // Порог может потребовать подстройки
            // Вместо стирания контента - показываем оверлей или перенаправляем
            document.body.innerHTML = '<div style="position:fixed;top:0;left:0;width:100%;height:100%;background:black;color:white;display:flex;align-items:center;justify-content:center;font-family:sans-serif;font-size:2rem;text-align:center;z-index:9999;">Отладка запрещена.<br>DevTools inspection is not allowed.</div>';
            // Или перенаправление:
